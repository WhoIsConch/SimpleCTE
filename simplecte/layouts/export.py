        ...
        yield sg.Tab(
            ...
            [
                [
                    ...
                    sg.InputText(key=f"-EXPORT_SEARCH_QUERY_{options[i].capitalize()}-", size=(30, 1),
                                 tooltip=" The query to search for in a field. "),
                ],
                [
                    sg.Text("Filter by:", size=(15, 1), tooltip=" The field to search for the query in. "),
                    sg.Combo([n.title() for n in get_field_keys(record=options[i].lower())],
                ...
                [
                    ...
                    sg.Combo([n.title() for n in get_sort_keys(record=options[i].lower())],
                             key=f"-EXPORT_SORT_TYPE_{options[i].capitalize()}-", size=(10, 1),
                             tooltip="The field to sort the results "
                                     "by. "),
                    sg.Checkbox("Descending", key=f"-EXPORT_SORT_DESCENDING_{options[i].capitalize()}-",
...
def get_export_layout():
    layout = [
        [
            ...
            sg.Column(
                ...
                layout=[
                    ...
                    [
                        sg.Checkbox("Resources", key="-EXPORT_RESOURCES-", default=True,
        ...
        [sg.pin(sg.Column(visible=False, justification="left", key="-EXPORT_FILTER_COL-", layout=[
            [
                sg.TabGroup(
                    layout=[list(_get_filter_options())]
                )
            ]
